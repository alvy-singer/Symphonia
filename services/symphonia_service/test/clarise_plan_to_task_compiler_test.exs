defmodule SymphoniaService.ClarisePlanToTaskCompilerTest do
  ...
  test "proposal persists a task proposal artifact and is deterministic", %{repository: repository} do
    ...
    assert proposal["metadata"]["generation_id"] == "milestone-001-plan-to-task-v1"
  ...
  test "confirmation writes generated tasks with source metadata and resolved dependencies", %{
    ...
    assert first["generationId"] == "milestone-001-plan-to-task-v1"
    ...
    assert first_markdown =~ "generation_id: milestone-001-plan-to-task-v1"
    assert first_markdown =~ "proposal_item_id: milestone-001-task-001"

    proposal = SpecWorkspace.read_artifact(repository, "task_proposal", "milestone-001-task-proposal")
  ...
  defp approved_milestone(repository, opts \\ []) do
    milestone = MilestoneLoop.start(repository, %{"title" => "Plan to task compiler"})["milestone"]
    ...
    if Keyword.get(opts, :decision?, false) do
      MilestoneLoop.decision(repository, milestone["id"], %{
        ...
        "body" => "# Keep task creation deliberate\n\n## Decision\n\nDo not start work automatically."
      })
    end
